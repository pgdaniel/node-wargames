...
function Wargames(server, options) {
	var self = this;

	if (!server) {
		throw new Error('You must pass the http server instance.');
	}
	...
	self.initiateCaching(function () {
		...
		self.io = self.initiateWebsocketServer();
	});
};
...
Wargames.prototype.sendUpdate = function (from, silent) {
	...
	if (from && from.lat) {
		...
		self.io.emit('message', JSON.stringify(returnObj));
	}
	else {
...
Wargames.prototype.sendLink = function (from, to, lastDitchEffort) {
	...
	if (from && to && from.lat && to.lat) {
		self.io.emit('message', JSON.stringify({
			'action': 'newLink',
			'from': self.getPublicUserInfo(from),
